...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "K_DEC: %d %d %s : "fmt, current->pid, __LINE__, __func__
...
static void isppyrdec_all_done(void *dec_handle)
{
	pr_debug("dec all done\n");
}
...
static isp_dec_isr isp_dec_isr_handler[32] = {
	[ISP_INT_DEC_ALL_DONE] = isppyrdec_all_done,
...
static int isppyrdec_cfg_store(struct isp_dec_pipe_dev *ctx, uint32_t store_block)
{
	...
	cmd = ((store->bypass & 0x1) << 0) |
		...
		((store->color_format & 0x7) << 4) |
		((store->mipi_en& 0x1) << 7) |
		((store->endian & 0x3) << 8) |
		((store->mono_en & 0x1) << 10) |
		((store->data_10b & 0x1) << 11) |
...
static int isppyrdec_cfg_offline(struct isp_dec_pipe_dev *ctx)
{
	...
	cmd = (dec_off_info->hor_padding_en & 0x1) |
		((dec_off_info->hor_padding_num & 0x7FFF) <<1) |
		((dec_off_info->ver_padding_en & 0x1) <<16) |
		((dec_off_info->ver_padding_num & 0x7F) <<17);
	FMCU_PUSH(fmcu, addr, cmd);
