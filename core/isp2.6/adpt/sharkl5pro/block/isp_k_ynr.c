...
int isp_k_update_ynr(void *handle)
{
	...
	pdst = &pctx->isp_using_param->ynr_info_v2;
	ynr_info = &pctx->isp_using_param->ynr_info_v2_base;
	if (ynr_info->bypass)
		return 0;
	...
	pr_debug("isp%d cen %d %d,  orig radius %d %d %d %d, base %d, new %d %d %d\n",
		pctx->ctx_id, center_x, center_y, ynr_info->radius, ynr_info->max_radius,
