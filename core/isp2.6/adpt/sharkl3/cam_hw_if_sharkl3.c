...
static int sharkl3_dcam_ioctl(void *handle,
	enum dcam_hw_cfg_cmd cmd, void *arg)
{
	int ret = 0;
	hw_ioctl_fun hw_ctrl = NULL;

	hw_ctrl = sharkl3_dcam_ioctl_get_fun(cmd);
	if (NULL != hw_ctrl)
...
static int sharkl3_isp_ioctl(void *handle, enum isp_hw_cfg_cmd cmd, void *arg)
{
	int ret = 0;
	hw_ioctl_fun hw_ctrl = NULL;

	hw_ctrl = sharkl3_isp_ioctl_get_fun(cmd);
	if (NULL != hw_ctrl)
		ret = hw_ctrl(handle, arg);
	else
		pr_debug("not support cmd %d", cmd);
...
struct cam_hw_info sharkl3_hw_info = {
	...
	.ip_isp = &sharkl3_isp,
	.dcam_ioctl = sharkl3_dcam_ioctl,
	.isp_ioctl = sharkl3_isp_ioctl,
};
