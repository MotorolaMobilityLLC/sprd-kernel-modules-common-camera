...
[_EISP_HIST] =    {"hist",     0x5410, 0, 1},
[_EISP_HIST2] =    {"hist2",     0x5510, 0, 1},
...
void set_common(struct sprd_cam_hw_info *hw)
{
	...
	for (i = ISP_CFAE_NEW_CFG0; i < ISP_CFAE_CSS_CFG11; i += 4)
		ISP_HREG_WR(i, 0);
	for (i = ISP_NLM_PARA; i < ISP_NLM_RADIAL_1D_ADDBACK00; i += 4)
		ISP_HREG_WR(i, 0);
...
void isp_set_ctx_common(struct isp_pipe_context *pctx)
{
	...
	ISP_REG_MWR(idx, ISP_COMMON_SCL_PATH_SEL,
			BIT_7 | BIT_6, 3 << 6);  /* store_dbg path off */
	ISP_REG_MWR(idx, ISP_COMMON_SCL_PATH_SEL,
	...
	if (pctx->fetch_path_sel)
		pr_info("Not support fbd feature %d\n", !fbd_raw->fetch_fbd_bypass);
