...
/*
********************************************************************************
* s_cambuf_allocsize[]  to found whether kenerl allocated iommu buf size for one
*    camera is free to zero when finish calling sprd_cam_buf_sg_table_put(). If
...
static int sprd_cambuf_map(struct cam_buf_info *buf_info)
{
	...
	for (i = 0; i < buf_info->num; i++) {
		...
		if (sprd_iommu_attach_device(buf_info->dev) == 0) {
			...
			if (buf_info->type == CAM_BUF_KERNEL_TYPE) {
				buf_info->kaddr[i] = (uint32_t *)
					sprd_ion_map_kernel(buf_info->dmabuf_p[i],
						0);
			...
			buf_info->iova[iova_i] = iommu_data.iova_addr + buf_info->offset[i];
		} else {
			unsigned long phys_addr = 0;

			ret = sprd_ion_get_phys_addr_by_db(buf_info->dmabuf_p[i],
					&phys_addr,
					&buf_info->size[i]);
			if (ret) {
			...
			buf_info->iova[iova_i] = phys_addr + buf_info->offset[i];
		}
		...
		if (iova_i)
			pr_debug("%s%d, 0x%x => 0x%x,0x%x total %d 0x%x\n",
				name,
				idx,
				(int)buf_info->iova[0],
				(int)buf_info->iova[1],
			...
			pr_debug("%s%d, 0x%x,0x%x total %d 0x%x\n",
				name,
				idx,
...
static int sprd_cambuf_unmap(struct cam_buf_info *buf_info)
{
	...
	for (i = 0; i < buf_info->num; i++) {
		...
		pr_debug("%s%d, iova 0x%x,0x%x total %d 0x%x\n",
			name,
			idx,
...
int sprd_cam_buf_sg_table_put(uint32_t idx)
{
	...
	list_for_each_entry_safe(fd_dma, fd_dma_next, buffer_list, list) {
		...
		if (fd_dma->type == CAM_BUF_USER_TYPE) {
			...
			sprd_cambuf_free(fd_dma->type,
				...
				fd_dma->size,
				(int)idx);
			pr_debug("del kernel buf: %p size 0x%x\n",
				fd_dma->dma_buf,
				fd_dma->size);
		}
		kfree(fd_dma);
	...
	pr_debug("cam%d total size 0x%x count %d\n",
		idx,
		atomic_read(&s_cambuf_allocsize[idx]),
		atomic_read(&s_cambuf_mapcnt[idx]));
	pr_debug("cam%d map size 0x%x 0x%x 0x%x x%x\n",
		idx,
