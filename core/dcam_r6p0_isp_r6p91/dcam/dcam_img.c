		 ...
		 struct sprd_img_iova *mapdata, void __user *arg)
{
	...
	if (sprd_iommu_attach_device(dev) == 0) {
		...
		if (ret)
			...
			ret = buffer_list_add(mapdata->fd, mapdata->sg_table, mapdata->size,
				IOMMU_BUF);
			if (ret){
		  ...
		  struct sprd_img_iova *unmapdata)
{
	...
	if (sprd_iommu_attach_device(dev) == 0 && !buffer_list_empty(IOMMU_BUF)) {
		if (!buffer_list_del(unmapdata->fd, unmapdata->sg_table,
				unmapdata->size, IOMMU_BUF)){
			     ...
			     unsigned long arg)
{
	...
	case SPRD_IMG_IO_SET_FRM_ID_BASE:
		...
		case DCAM_PATH1:
		case DCAM_PATH2:
			dev->dcam_cxt.dcam_path[channel_id].frm_id_base =
				frame_base_id;
			break;
		default:
			pr_err("Wrong channel ID, %d .\n",
	...
	case SPRD_IMG_IO_PATH_PAUSE:
		uparam = (struct sprd_img_parm __user *)arg;
		ret = get_user(channel_id, &uparam->channel_id);
		ret |= get_user(reserved, &uparam->reserved[0]);
		if (ret || (channel_id >= DCAM_PATH_NUM)) {
			pr_err("fail to get user info.\n");
			goto exit;
		}
