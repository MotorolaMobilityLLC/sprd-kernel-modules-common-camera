		...
		if (buf_type == IOMMU_BUF) {
			...
			if (ret)
				pr_err("unmap fail, ret %d, fd 0x%x iova 0x%x size 0x%zx sg %x\n",
					ret, fd_buf->fd, fd_buf->size, (unsigned int)iommu_data.iova_addr, fd_buf->buf);
			DMABUF_TRACE("put:type %d 0x%x 0x%x\n", buf_type, fd_buf->fd, fd_buf->dmabuf);
			dma_buf_put(fd_buf->dmabuf);
		} else {
			DMABUF_TRACE("put:type %d 0x%x 0x%x\n", buf_type, fd_buf->fd, fd_buf->dmabuf);
			dma_buf_put(fd_buf->dmabuf);
		}
		DMABUF_TRACE("del:type %d %x %x %x\n", buf_type, fd_buf->fd, fd_buf->buf, fd_buf->dmabuf);
		kfree(fd_buf);
	}
...
int pfiommu_get_addr(struct pfiommu_info *pfinfo)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (!pfinfo->is_secure && sprd_iommu_attach_device(pfinfo->dev) == 0) {
			...
			DMABUF_TRACE("%d,buf %x,dma buf %d,fd %x", i, pfinfo->buf[i], pfinfo->dmabuf_p[i], pfinfo->mfd[i]);
...
int pfiommu_free_addr(struct pfiommu_info *pfinfo)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (!pfinfo->is_secure && sprd_iommu_attach_device(pfinfo->dev) == 0) {
			...
			iommu_data.ch_type = SPRD_IOMMU_FM_CH_RW;
			iommu_data.buf = NULL;
			DMABUF_TRACE("%d,buf %x,dma buf %d,fd %x,iova %x,sg_table %x", i, pfinfo->buf[i], pfinfo->dmabuf_p[i], pfinfo->mfd[i], pfinfo->iova[i], pfinfo->table[i]);
	...
	for (i = 0; i < 2; i++) {
		...
		if (!pfinfo->is_secure && sprd_iommu_attach_device(pfinfo->dev) == 0) {
			...
			ret = sprd_iommu_unmap(pfinfo->dev,
					&iommu_data);
			if (ret) {
				pr_err("failed to free iommu %d\n iova %s cid %d", i, pfinfo->iova[i], cid);
				//dump_stack();
				return -EFAULT;
			} else {
				pfinfo->iova[i] = 0;
