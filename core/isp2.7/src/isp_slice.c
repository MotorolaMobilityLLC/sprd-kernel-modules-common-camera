		...
		void *cfg_in, struct isp_slice_context *slc_ctx)
{
	...
	uint32_t start_row, end_row, start_col, end_col;
	...
	for (i = 0; i < SLICE_NUM_MAX; i++, cur_slc++) {
		...
		for (j = 0; j < AFBC_PATH_NUM; j++) {
			...
			if (slc_scaler->scaler_bypass == 1) {
				...
				slc_afbc_store->size.w = end_col - start_col + 1 - overlap_left - overlap_right;
				slc_afbc_store->size.h = end_row - start_row + 1 - overlap_up - overlap_down;
				...
				while ((int)(tmp_slice_id - 1) >=
					...
					tmp_slice_id--;
					slice_start_col +=
						(cur_slc[tmp_slice_id].slice_afbc_store[j].size.w +
							AFBC_PADDING_W_YUV420_scaler - 1) /
							AFBC_PADDING_W_YUV420_scaler *
							AFBC_PADDING_W_YUV420_scaler;
				}
			}
...
static int set_slice_spath_afbc_store(
		struct isp_fmcu_ctx_desc *fmcu,
		uint32_t path_en,
		uint32_t ctx_idx,
		enum isp_sub_path_id spath_id,
...
{
	uint32_t addr = 0, cmd = 0;
	unsigned long base = fbc_store_base[spath_id];

	if (!path_en) {
		/* bit0 bypass store */
		addr = ISP_GET_REG(ISP_AFBC_STORE_PARAM) + base;
		cmd = 1;
		FMCU_PUSH(fmcu, addr, cmd);
		return 0;
	}
	addr = ISP_GET_REG(ISP_AFBC_STORE_PARAM) + base;
	cmd = ISP_REG_RD(ctx_idx, base + ISP_AFBC_STORE_PARAM) & ~1;
	FMCU_PUSH(fmcu, addr, cmd);

	addr = ISP_GET_REG(ISP_AFBC_STORE_SLICE_SIZE) + base;
	cmd = ((slc_afbc_store->size.h & 0xFFFF) << 16) |
			(slc_afbc_store->size.w & 0xFFFF);
	FMCU_PUSH(fmcu, addr, cmd);

	addr = ISP_GET_REG(ISP_AFBC_STORE_BORDER) + base;
	...
	addr = ISP_GET_REG(ISP_AFBC_STORE_SLICE_HEADER_OFFSET_ADDR) + base;
	cmd = slc_afbc_store->slice_offset;
	FMCU_PUSH(fmcu, addr, cmd);

	cmd = slc_afbc_store->yheader_addr;
	addr = ISP_GET_REG(ISP_AFBC_STORE_SLICE_Y_HEADER) + base;
	FMCU_PUSH(fmcu, addr, cmd);

	cmd = slc_afbc_store->yaddr;
	addr = ISP_GET_REG(ISP_AFBC_STORE_SLICE_Y_ADDR) + base;
	FMCU_PUSH(fmcu, addr, cmd);
		...
		for (j = 0; j < ISP_SPATH_NUM; j++) {
			...
			if (j < AFBC_PATH_NUM) {
				...
				set_slice_spath_afbc_store(fmcu,
					cur_slc->path_en[j], ctx_idx, j, slc_afbc_store);
			}
		}
...
static int update_slice_spath_afbc_store(
		uint32_t path_en,
		uint32_t ctx_id,
		enum isp_sub_path_id spath_id,
		struct slice_afbc_store_info *slc_afbc_store)
{
	uint32_t addr = 0, cmd = 0;
	uint32_t base = (uint32_t)fbc_store_base[spath_id];

	if (!path_en) {
		/* bit0 bypass store */
		addr = ISP_AFBC_STORE_PARAM + base;
		cmd = 1;
		ISP_REG_WR(ctx_id, addr, cmd);
		return 0;
	}
	addr = ISP_AFBC_STORE_PARAM + base;
	cmd = ISP_REG_RD(ctx_id, base + ISP_AFBC_STORE_PARAM) & ~1;
	ISP_REG_WR(ctx_id, addr, cmd);

	addr = ISP_AFBC_STORE_SLICE_SIZE + base;
	cmd = ((slc_afbc_store->size.h & 0xFFFF) << 16) |
			(slc_afbc_store->size.w & 0xFFFF);
	ISP_REG_WR(ctx_id, addr, cmd);

	addr = ISP_AFBC_STORE_BORDER + base;
	cmd = (slc_afbc_store->border.up_border& 0xFF) |
			((slc_afbc_store->border.down_border& 0xFF) << 8) |
			((slc_afbc_store->border.left_border& 0xFF) << 16) |
			((slc_afbc_store->border.right_border & 0xFF) << 24);
	...
	addr = ISP_AFBC_STORE_SLICE_HEADER_OFFSET_ADDR + base;
	cmd = slc_afbc_store->slice_offset;
	ISP_REG_WR(ctx_id, addr, cmd);

	cmd = slc_afbc_store->yheader_addr;
	addr = ISP_AFBC_STORE_SLICE_Y_HEADER + base;
	ISP_REG_WR(ctx_id, addr, cmd);

	cmd = slc_afbc_store->yaddr;
	addr = ISP_AFBC_STORE_SLICE_Y_ADDR + base;
	ISP_REG_WR(ctx_id, addr, cmd);
