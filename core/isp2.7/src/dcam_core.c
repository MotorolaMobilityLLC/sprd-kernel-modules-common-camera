...
void dcam_debug_trace(struct dcam_pipe_dev *dev)
{
...
#endif
static int dcam_start_l5ro(struct dcam_pipe_dev *dev)
{
	int ret = 0;
	uint32_t idx = dev->idx;
...
#if 1
static int dcam_set_mipi_cap_l5pro(struct dcam_pipe_dev *dev,
				struct dcam_mipi_info *cap_info)
{
	int ret = 0;
	uint32_t idx = dev->idx;
...
static int dcam_offline_start_frame(void *param)
{
	...
	for (i  = 0; i < DCAM_PATH_MAX; i++) {
		...
		if (ret == 0) {
			...
			dcam_start_path_l5pro(dev, path);
		}
	}
...
#if 1
	ret = dcam_set_mipi_cap_l5pro(dev, &dev->cap_info);
	if (ret < 0) {
