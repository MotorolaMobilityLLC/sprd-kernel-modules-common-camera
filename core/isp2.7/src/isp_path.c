...
int isp_cfg_path_base(struct isp_path_desc *path, void *param)
{
	...
	enum isp_afbc_path afbc_path_id =
		(enum isp_afbc_path)path->spath_id;
...
static int set_path_afbc_store(struct isp_path_desc *path)
{
	int ret = 0;
	uint32_t val = 0;
	uint32_t idx = path->attach_ctx->ctx_id;
	struct isp_afbc_store_info *afbc_store_info = &path->afbc_store;
	unsigned long addr = fbc_store_base[path->spath_id];
		...
		struct camera_frame *frame)
{
	...
	idx = pctx->ctx_id;
	afbc_addr = fbc_store_base[path->spath_id];

	yuv_addr[0] = frame->buf.iova[0];
	yuv_addr[1] = frame->buf.iova[1];

	pr_info("afbc addr %lx %lx\n", yuv_addr[0], yuv_addr[1]);

	if (yuv_addr[1] == 0)
		yuv_addr[1] = yuv_addr[0] + afbc_store->header_offset;

	pr_info("path %d afbc addr %lx %lx\n",
		path->spath_id, yuv_addr[0], yuv_addr[1]);

	ISP_REG_WR(idx, afbc_addr + ISP_AFBC_STORE_SLICE_Y_HEADER, yuv_addr[0]);
	ISP_REG_WR(idx, afbc_addr + ISP_AFBC_STORE_SLICE_Y_ADDR, yuv_addr[1]);

	path->afbc_store.yheader= yuv_addr[0];
	path->afbc_store.yaddr = yuv_addr[1];

	pr_info("afbc done %x %x\n", path->afbc_store.yheader,
		path->afbc_store.yaddr);
