...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "PYRDEC_K_BLK: %d %d %s : "fmt, current->pid, __LINE__, __func__

static irqreturn_t pyrdec_isr_root(int irq, void *priv)
{
	uint32_t irq_line = 0;
	uint32_t err_mask = 0;
	...
	if (unlikely(irq != ctx->irq_no)) {
		pr_err("fail to match pyr dec irq %d %d\n", irq, ctx->irq_no);
		return IRQ_NONE;
	}

	err_mask = PYR_DEC_INT_LINE_MASK_ERR;
	irq_line = ISP_HREG_RD(ISP_DEC_INT_BASE + ISP_INT_INT0);
	ISP_HREG_WR(ISP_DEC_INT_BASE + ISP_INT_CLR0, irq_line);

	pr_debug("pyrdec irq status: 0x%x\n", irq_line);
...
{
	int ret = 0;
	struct pyrdec_pipe_dev *ctx = NULL;

	if (!handle) {
		pr_err("fail to pyr_dec handle NULL\n");
		return -EFAULT;
	}

	ctx = (struct pyrdec_pipe_dev *)handle;
	ctx->isr_func = pyrdec_isr_root;

	return ret;
}

static int pyrdec_cfg_fetch(struct pyr_dec_node *node)
{
	int ret = 0;
	uint32_t addr = 0, cmd = 0, base = 0, color_format = 0;
	struct isp_fmcu_ctx_desc *fmcu = NULL;
	struct pyr_dec_fetch_info *dec_fetch = NULL;
	...
	dec_fetch = &node->fetch_dec_info;
	dec_afbd_fetch = &node->yuv_afbd_info;
	dec_dev = node->pyrdec_dev;
	fmcu = (struct isp_fmcu_ctx_desc *)dec_dev->fmcu_handle;
	fetch_slc = &node->slices[node->cur_slice_id].slice_fetch;
	...
	if (node->cur_layer_id == 0 && node->fetch_path_sel) {
		base = ISP_YUV_DEC_AFBD_FETCH_BASE;

		if (node->yuv_afbd_info.data_bits == CAM_8_BITS)
			node->yuv_afbd_info.afbc_mode = AFBD_FETCH_8BITS;
		else if (node->yuv_afbd_info.data_bits == CAM_10_BITS)
			node->yuv_afbd_info.afbc_mode = AFBD_FETCH_10BITS;
		else
			pr_debug("No use afbc mode.\n");

		addr = ISP_GET_REG(ISP_COMMON_SCL_PATH_SEL);
		cmd = ISP_REG_RD(node->isp_node_cfg_id, ISP_COMMON_SCL_PATH_SEL);
		cmd = cmd | (1 << 14);
		...
		cmd = (node->yuv_afbd_info.fetch_fbd_bypass & 0x1) |
			(0x1 << 1) | (0x1 << 3) | ((node->yuv_afbd_info.afbc_mode & 0x1F) << 4);
		FMCU_PUSH(fmcu, addr, cmd);

		addr = ISP_GET_REG(ISP_AFBD_FETCH_HBLANK_TILE_PITCH ) + base;
		cmd = ((node->yuv_afbd_info.tile_num_pitch & 0x7FF) << 16) | (0x8000);
		FMCU_PUSH(fmcu, addr, cmd);
		...
		cmd = node->yuv_afbd_info.frame_header_base_addr;
		FMCU_PUSH(fmcu, addr, cmd);
...
static int pyrdec_cfg_store(struct pyr_dec_node *node, uint32_t store_block)
{
	int ret = 0;
	uint32_t addr = 0, cmd = 0, base = 0;
	uint32_t color_format = 0, data_10b = 0;
	struct isp_fmcu_ctx_desc *fmcu = NULL;
	struct pyr_dec_store_info *store = NULL;
	struct slice_store_info *store_slc = NULL;
	struct pyrdec_pipe_dev *dec_dev = NULL;

	if (node == NULL) {
		pr_err("fail to get pyr dec node.\n");
		return -EFAULT;
	}
	...
	fmcu = (struct isp_fmcu_ctx_desc *)dec_dev->fmcu_handle;
	switch (store_block) {
	case PYR_DEC_STORE_DCT:
		base = PYR_DCT_STORE_BASE;
		store = &node->store_dct_info;
		store_slc = &node->slices[node->cur_slice_id].slice_dct_store;
		break;
	case PYR_DEC_STORE_DEC:
		base = PYR_DEC_STORE_BASE;
		store = &node->store_dec_info;
		store_slc = &node->slices[node->cur_slice_id].slice_dec_store;
		break;
	default:
...
static int pyrdec_cfg_offline(struct pyr_dec_node *node)
{
	int ret = 0;
	uint32_t addr = 0, cmd = 0;
	struct isp_fmcu_ctx_desc *fmcu = NULL;
	struct pyr_dec_offline_info *dec_off_info = NULL;
	struct slice_pyr_dec_node_info *pyr_dec_slc = NULL;
	...
	dec_dev = node->pyrdec_dev;
	fmcu = (struct isp_fmcu_ctx_desc *)dec_dev->fmcu_handle;
	dec_off_info = &node->offline_dec_info;
	pyr_dec_slc = &node->slices[node->cur_slice_id].slice_pyr_dec;
	hw = node->hw;
	...
	if (node->fetch_path_sel && node->cur_layer_id == 0) {
...
static int pyrdec_cfg_dct_ynr(struct pyr_dec_node *node)
{
	int ret = 0;
	...
	if (node->cur_layer_id == 0) {
		...
		cmd = (dct_ynr->start[node->cur_slice_id].w & 0xFFFF) |
			((dct_ynr->start[node->cur_slice_id].h & 0xFFFF) << 16);
		FMCU_PUSH(fmcu, addr, cmd);
		...
		if (node->store_dct_info.color_format == CAM_YVU420_2FRAME) {
...
int pyr_dec_config(void *handle)
{
	...
	pyrdec_cfg_fetch(node);
	if (node->hw->prj_id == QOGIRN6pro)
		pyrdec_cfg_store(node, PYR_DEC_STORE_DCT);
	pyrdec_cfg_store(node, PYR_DEC_STORE_DEC);
	if (node->hw->prj_id == QOGIRN6pro)
		pyrdec_cfg_dct_ynr(node);
	pyrdec_cfg_offline(node);
