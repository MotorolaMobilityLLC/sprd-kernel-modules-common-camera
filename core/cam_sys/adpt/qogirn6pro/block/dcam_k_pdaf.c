...
static int isp_k_pdaf_bypass(struct isp_io_param *param, struct dcam_isp_k_block *p)
{
	int ret = 0;
	uint32_t bypass = 0;
	...
	struct isp_io_param *param, struct dcam_isp_k_block *p)
{
	...
	dcam_online_port_skip_num_set(p->dev, idx, DCAM_PATH_PDAF, p->pdaf.skip_num);
	pr_debug("skip num %d\n", p->pdaf.skip_num);
