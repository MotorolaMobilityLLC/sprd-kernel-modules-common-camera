...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "POSTCNR: %d %d %s : " fmt, current->pid, __LINE__, __func__
...
int isp_k_update_post_cnr(void *handle)
{
	...
	post_cnr_info = &block_param->isp_using_param->post_cnr_h_info;
	...
	pr_debug("isp%d,cen %d %d, base %d, factor %d, radius %d\n",
		block_param->cfg_id, center_x, center_y, post_cnr_info->radius_base, param_post_cnr_info->base_radius_factor, radius);
