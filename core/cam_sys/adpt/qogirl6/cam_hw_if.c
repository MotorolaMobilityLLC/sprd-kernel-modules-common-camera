...
static int camhw_get_block_type(void *handle, void *arg)
{
	...
	if (hw->ip_isp->rgb_gtm_support && (blk_type->sub_block == DCAM_BLOCK_GTM))
...
static struct cam_hw_ip_info dcam[DCAM_ID_MAX] = {
	[DCAM_ID_0] = {
		...
		.raw_fmt_support[0] = CAM_RAW_14,
		.raw_fmt_support[1] = CAM_RAW_8,
		.raw_fmt_support[2] = CAM_RAW_HALFWORD_10,
		.raw_fmt_support[3] = CAM_RAW_PACK_10,
