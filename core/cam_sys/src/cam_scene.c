		...
		struct dcam_online_node_desc *dcam_online_desc)
{
	...
	for (i = 0; i < PORT_DCAM_OUT_MAX; i++) {
		...
		cam_block_valid_fmt_get(&channel->ch_uinfo.dcam_raw_fmt, hw->ip_dcam[0]->raw_fmt_support[0]);
		...
		if (pipeline_type == CAM_PIPELINE_SENSOR_RAW
			...
			cam_block_valid_fmt_get(&channel->ch_uinfo.sensor_raw_fmt, hw->ip_dcam[0]->sensor_raw_fmt);
	...
	struct isp_node_desc *isp_node_description)
{
	...
	if (channel->ch_uinfo.is_high_fps)
		...
		isp_node_description->blkparam_node_num = cam_scene_buffers_alloc_num(channel, module) + 1;
