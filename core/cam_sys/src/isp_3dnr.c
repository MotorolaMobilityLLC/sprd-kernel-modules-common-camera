		...
		enum isp_3dnr_cfg_cmd cmd, void *param)
{
	...
	case ISP_3DNR_CFG_BLEND_INFO:
		nr3_ctx->isp_block = (struct dcam_isp_k_block *)param;
		break;
	case ISP_3DNR_CFG_MEMCTL_STORE_INFO:
		...
		nr3_ctx->nr3_store.color_format = fetch_info->fetch_3dnr_fmt;
		nr3_ctx->nr3_store.mipi_en = 0;
		nr3_ctx->nr3_store.data_10b = (cam_data_bits(fetch_info->fetch_3dnr_fmt) == 8) ? 0 : 1;
		break;
