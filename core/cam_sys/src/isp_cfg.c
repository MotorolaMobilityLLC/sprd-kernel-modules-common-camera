...
static int ispcfg_cctx_buf_init(struct isp_cfg_ctx_desc *cfg_ctx)
{
	...
	ispcfg_cctx_page_sw_buf_addr_init(cfg_ctx, (void *)ion_buf_cached->addr_k);
	...
	pr_debug("cmd1 sw: 0x%lx, size:0x%x\n",
		ion_buf_cached->addr_k, (int)ion_buf_cached->size);
...
int isp_cfg_ctx_reg_buf_debug_show(void *param)
{
	...
	for (i = 0; i < cfg_map_size; i++) {
		...
		item = cfg_map[i];
		offset = item & 0x3ffff;
		size = ((item >> 18) & 0x3ffff) * 4;
		for (addr = offset; addr < (offset + size); addr += 4)
