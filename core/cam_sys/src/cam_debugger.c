...
static int camdebugger_fbc_control_read(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "\nUsage:\n");
	seq_printf(s, "         echo val > fbc_control\n");
	seq_printf(s, "The different bits represent fbc switch in different path\n");
	seq_printf(s, "\nExample:\n");
	seq_printf(s, "         echo 6 > fbc_control   // bit 110, bypass full and raw path fbc\n");
	seq_printf(s, "         echo 3 > fbc_control   // bit 011, bypass bin and full path fbc\n");
...
static int camdebugger_dump_control_read(struct seq_file *s, void *unused)
{
	...
	seq_printf(s, "      1.setenforce 0\n");
	seq_printf(s, "      2.echo val > dump_control\n");
	seq_printf(s, "  val = (pipeline_type << 3)|(node_type << 2)|(port_id << 1)|(state << 0)\n");
	seq_printf(s, "\nExample:\n");
	seq_printf(s, "  Dump capture_pipeline dcam_online_node full_port enable:\n");
	seq_printf(s, "      setenforce 0\n");
	seq_printf(s, "      echo 2021 > dump_control\n");
	seq_printf(s, "  2->cap pipeline, 0->dcam online node, 2->full port, 1->work\n");
	seq_printf(s, "\nDump Info:\n");
...
static int camdebugger_dump_control_open(struct inode *inode, struct file *file)
{
	return single_open(file, camdebugger_dump_control_read, inode->i_private);
}

static ssize_t camdebugger_dump_control_write(struct file *filp,
	const char __user *buffer, size_t count, loff_t *ppos)
{
	int ret = 0;
	char msg[8];
...
static const struct file_operations dump_control_ops = {
	.owner = THIS_MODULE,
	.open = camdebugger_dump_control_open,
	.read = seq_read,
	.write = camdebugger_dump_control_write,
};
...
static ssize_t camdebugger_rawcap_frgb_read(struct file *filp,
	char __user *buffer, size_t count, loff_t *ppos)
{
	const char *desc = "0: rawcap mode, 1: frgb mode";
	char buf[64] = {0};
	char *str = "Example: echo 1 > rawcap_frgb_switch";
...
static ssize_t camdebugger_rawcap_frgb_write(struct file *filp,
	const char __user *buffer, size_t count, loff_t *ppos)
{
	int ret = 0;
	char msg[8] = {0};
	...
	g_dbg_raw2frgb_switch = val;
	pr_info("set raw2frgb_switch %u\n", g_dbg_raw2frgb_switch);

	return count;
}

static const struct file_operations rawcap_frgb_switch_ops = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.read = camdebugger_rawcap_frgb_read,
	.write = camdebugger_rawcap_frgb_write,
};
...
static int camdebugger_dcam_init(struct camera_debugger *debugger)
{
	...
	if (!debugfs_create_file("zoom_mode", 0664,
		...
		ret |= BIT(7);
	if (!debugfs_create_file("dump_control", 0664,
		pd, NULL, &dump_control_ops))
		ret |= BIT(9);
	...
	if (!debugfs_create_file("memory_leak_control", 0664,
		...
		ret |= BIT(22);
	if (!debugfs_create_file("rawcap_frgb_switch", 0664,
		pd, debugger, &rawcap_frgb_switch_ops))
		ret |= BIT(23);
...
static int camdebugger_isp_init(struct camera_debugger *debugger)
{
	...
	static struct cam_debug_bypass isp_debug_bypass[4] = {0};
	char dirname[32] = {0};
	int i = 0;
