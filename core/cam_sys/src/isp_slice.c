		...
		void *cfg_in, struct isp_slice_context *slc_ctx)
{
	...
	for (idx = 0; idx < SLICE_NUM_MAX; idx++, cur_slc++) {
		...
		slc_3dnr_memctrl->data_toyuv_en = mem_ctrl->data_toyuv_en;
		slc_3dnr_memctrl->back_toddr_en = mem_ctrl->back_toddr_en;
		...
		struct isp_hw_context *pctx_hw)
{
	...
	if (pctx_hw->nr3_fbc_fbd) {
		...
		nr3_fbc_slice.fbc_store->ctx_id = pctx_hw->cfg_id;
		fbd.fbd_fetch->ctx_id = pctx_hw->cfg_id;
...
int isp_slice_fmcu_cmds_set(void *fmcu_handle, void *ctx)
{
	...
	struct isp_slice_desc *cur_slc = NULL;
	struct slice_store_info *slc_store = NULL;
	...
	pctx_hw = VOID_PTR_TO(ctx, struct isp_hw_context);
	fmcu = VOID_PTR_TO(fmcu_handle, struct isp_fmcu_ctx_desc);
	rec_ctx = (struct isp_rec_ctx_desc *)pctx_hw->rec_handle;
	cfg_id = pctx_hw->cfg_id;
	hw = pctx_hw->hw;
	hw_ctx_id = pctx_hw->hw_ctx_id;
	pr_debug("get hw context id=%d\n", hw_ctx_id);
	wmode = pctx_hw->wmode;
	slc_ctx = (struct isp_slice_context *)pctx_hw->slice_ctx;
	if (!slc_ctx || slc_ctx->slice_num < 1) {
	...
	for (i = 0; i < SLICE_NUM_MAX; i++, cur_slc++) {
		...
		if (!slc_ctx->pyr_rec_eb || rec_ctx->fetch_path_sel != 1) {
			if (!cur_slc->slice_fbd_yuv.fetch_fbd_bypass){
		...
		ispslice_3dnr_set(fmcu, cur_slc, hw, pctx_hw);
...
int isp_slice_update(void *slice_ctx, void *hw_info, uint32_t cfg_id, uint32_t slice_id)
{
	...
	if (!slice_ctx || !hw_info) {
		pr_err("fail to get input ptr, null.\n");
		return -EFAULT;
	}

	hw = (struct cam_hw_info *)hw_info;
	slc_ctx = (struct isp_slice_context *)slice_ctx;
	if (slc_ctx->slice_num < 1) {
		pr_err("fail to use slices, not support here.\n");
		return -EINVAL;
	}
