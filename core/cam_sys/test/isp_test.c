...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "CAMT: %d %d %s : " fmt, current->pid, __LINE__, __func__
...
static int camt_isp_cfg_init(struct ispt_context *ctx, struct camt_info *info)
{
	...
	pr_info("cmd sw: %p, hw: 0x%lx, size:0x%x\n",
		sw_addr, hw_addr, (int)ion_buf->size);
	...
	struct camt_info *info)
{
	...
	fetch->sec_mode = 0;
	fetch->fetch_fmt = CAM_RAW_HALFWORD_10;
	...
	struct camt_info *info, int i)
{
	...
	store->color_fmt = CAM_YUV420_2FRAME;
	store->bypass = 0;
...
int ispt_init(struct cam_hw_info *hw, struct camt_info *info)
{
	...
	cxt = cam_buf_kernel_sys_kzalloc(sizeof(struct ispt_context), GFP_KERNEL);
	if (cxt == NULL) {
...
int ispt_start(struct camt_info *info)
{
	...
	hw->isp_ioctl(hw, ISP_HW_CFG_DEFAULT_PARA_CFG, &ctx_id);
