...
static int sprd_cam_domain_eb(struct camsys_power_info *pw_info)
{
	...
	regcache_cache_only(pw_info->u.l5pro.ahb_map, false);
	udelay(200);
...
static int sprd_cam_domain_disable(struct camsys_power_info *pw_info)
{
	...
	regcache_cache_only(pw_info->u.l5pro.ahb_map, true);
	udelay(200);
...
static int sprd_cam_pw_on(struct camsys_power_info *pw_info)
{
	...
	for (i = 0; i < 30; i++) {
		...
		for (j = 0; j < 5; j++) {
			ret = regmap_read_mmsys(&pw_info->u.l5pro.regs[PWR_STATUS0], &power_state);
...
static long sprd_campw_init(struct platform_device *pdev, struct camsys_power_info *pw_info)
{
	...
	pw_info->u.l5pro.ahb_map = syscon_regmap_lookup_by_phandle(np, "sprd,cam-ahb-syscon");
	if (IS_ERR_OR_NULL(pw_info->u.l5pro.ahb_map))
