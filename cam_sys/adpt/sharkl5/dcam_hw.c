...
static int dcamhw_record_addr(void *handle, void *arg)
{
	...
	hw_ctx->frame_addr[count][DCAM_RECORD_PORT_AEM_SHORT] = DCAM_REG_RD(idx, DCAM_AEM_SHORT_WADDR);
...
static int dcamhw_reset(void *handle, void *arg)
{
	...
	struct cam_hw_ip_info *ip = NULL;
	struct cam_hw_info *hw = NULL;
	uint32_t i = 0, time_out = 0, flag = 0, bypass = 1, eb = 0;
...
{
	int time_out = 0, hw_ctx_id = 0, dbg_sts_reg = 0;
	if (!handle || !arg) {
		pr_err("fail to get input arg\n");
		return -EFAULT;
	}
