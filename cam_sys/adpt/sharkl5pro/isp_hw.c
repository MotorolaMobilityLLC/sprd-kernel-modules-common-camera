...
static int isphw_path_store(void *handle, void *arg)
{
	...
	ISP_REG_MWR(idx, addr + ISP_STORE_PARAM, 0xF0, (val << 4));
...
static int isphw_fetch_set(void *handle, void *arg)
{
	...
	pr_debug("camca, isp sec mode=%d , pitch_ch0=0x%x, 0x%x, 0x%x, format:%d\n",
		fetch->sec_mode,
...
static int isphw_block_param_config(void *handle, void *arg)
{
	...
	blkpm_ptr = (struct isp_pipeline_param_l5pro *)arg;
	...
	if (blkpm_ptr->imbalance_param.update_flag) {
		memcpy(&p->imbalance_info_base, &blkpm_ptr->imbalance_param, sizeof(struct isp_dev_nlm_imblance_v1));
		p->imbalance_info_base.isupdate = 1;
	}
