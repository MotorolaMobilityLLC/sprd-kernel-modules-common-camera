...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "UVD : %d %d %s : " fmt, current->pid, __LINE__, __func__

int isp_k_uvd_block(struct dcam_isp_k_block *isp_k_param, uint32_t idx)
{
	int ret = 0;
	uint32_t val = 0;
	struct isp_dev_uvd_info *uvd_info = NULL;
	if (isp_k_param->uvd_info.isupdate == 0)
		return ret;
	uvd_info = &isp_k_param->uvd_info;
	isp_k_param->uvd_info.isupdate = 0;
	...
	struct dcam_isp_k_block *isp_k_param)
{
	...
	case ISP_PRO_UVD_BLOCK:
		ret = copy_from_user((void *)uvd_info, param->property_param, sizeof(struct isp_dev_uvd_info));
		if (ret != 0) {
			pr_err("fail to copy from user, ret = %d\n", ret);
			return ret;
		}
		isp_k_param->uvd_info.isupdate = 1;
		break;
	default:
...
{
	int ret = 0;
	if (isp_k_param->uvd_info.isupdate == 1) {
		memcpy(&param_block->uvd_info, &isp_k_param->uvd_info, sizeof(struct isp_dev_uvd_info));
		isp_k_param->uvd_info.isupdate = 0;
		param_block->uvd_info.isupdate = 1;
	}
