	...
	case DCAM_ID_1:
		if (project_mode == 0) {
			...
			if (src.w > DCAM1_3DNR_ME_WIDTH_MAX) {
				lbuf = 1;
				/* when dcam1 use lbuf of dacm2, should bypass dcam2 */
				DCAM_REG_MWR(DCAM_ID_2, NR3_FAST_ME_PARAM, BIT_0, BIT_0);
...
int dcam_k_3dnr_me(struct dcam_isp_k_block *param)
{
	...
	hw_path = &hw_ctx->hw_path[DCAM_PATH_3DNR];
	rect.x = hw_path->in_trim.start_x;
	rect.y = hw_path->in_trim.start_y;
	rect.w = hw_path->in_trim.size_x;
	rect.h = hw_path->in_trim.size_y;
	if ((rect.x + rect.w) <= hw_ctx->cap_info.cap_size.size_x &&
