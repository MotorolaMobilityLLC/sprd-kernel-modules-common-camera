...
abnormal_reg_trace:
	...
	for (addr = DCAM_IP_REVISION; addr <= DCAM_LSC_WEI_LAST1; addr += 16) {
...
static int isphw_fetch_set(void *handle, void *arg)
{
	...
	ISP_REG_MWR(idx, ISP_FETCH_PARAM, (0xF << 4), val << 4);
	if (fetch->fetch_fmt == CAM_FULL_RGB10)
...
static int isphw_ltm_slice_set(void *handle, void *arg)
{
	...
	if (!map->bypass) {
		addr = ISP_GET_REG(ISP_LTM_MAP_RGB_BASE + ISP_LTM_MAP_PARAM1);
		...
		addr = ISP_GET_REG(ISP_LTM_MAP_RGB_BASE + ISP_LTM_MAP_PARAM3);
		...
		addr = ISP_GET_REG(ISP_LTM_MAP_RGB_BASE + ISP_LTM_MAP_PARAM4);
		cmd = map->mem_addr;
		FMCU_PUSH(fmcu, addr, cmd);
	}

	if (!hist->bypass) {
		addr = ISP_GET_REG(ISP_LTM_HIST_RGB_BASE + ISP_LTM_ROI_START);
		cmd = ((hist->roi_starty & 0x1FFF) << 16) | (hist->roi_startx & 0x1FFF);
		FMCU_PUSH(fmcu, addr, cmd);

		addr = ISP_GET_REG(ISP_LTM_HIST_RGB_BASE + ISP_LTM_TILE_RANGE);
		...
		addr = ISP_GET_REG(ISP_LTM_HIST_RGB_BASE + ISP_LTM_ADDR);
		cmd = hist->mem_addr;
		FMCU_PUSH(fmcu, addr, cmd);

		addr = ISP_GET_REG(ISP_LTM_HIST_RGB_BASE + ISP_LTM_PITCH);
		hist->ddr_wr_num = (hist->tile_num_x_minus1 + 1) * 16;
		cmd = (((hist->ddr_wr_num * 2) & 0x1FF) << 16) | (hist->ddr_pitch & 0xFFFF);
		FMCU_PUSH(fmcu, addr, cmd);
	}
...
static int isphw_block_param_config(void *handle, void *arg)
{
	...
	blkpm_ptr = (struct isp_pipeline_param_l6 *)arg;
	...
	if (blkpm_ptr->imbalance_param.update_flag) {
		...
		p->imbalance_info_base2.isupdate = 1;
	}
	...
	if (blkpm_ptr->hsv3_param.update_flag) {
	...
	if (blkpm_ptr->gtm_param.update_flag) {
		memcpy(&p->gtm.gtm_info, &blkpm_ptr->gtm_param, sizeof(struct dcam_dev_raw_gtm_block_info));
		p->gtm.gtm_info.isupdate = 1;
	}
