...
int dcam_k_lsc_block(struct dcam_isp_k_block *p)
{
	...
	if ((info->weight_num_x > DCAM_LSC_WEIGHT_TAB_SIZE) || (info->weight_num_y > DCAM_LSC_WEIGHT_TAB_SIZE)) {
		pr_err("fail to get weight length wrong %d %d\n", info->weight_num_x, info->weight_num_y);
		ret = -EPERM;
		goto exit;
	}
