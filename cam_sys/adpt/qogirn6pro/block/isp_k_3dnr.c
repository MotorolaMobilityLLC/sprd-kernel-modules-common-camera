...
static void isp_3dnr_blend_param_debug(struct isp_dev_3dnr_info_v1 *pnr3)
{
	uint32_t i = 0;
				  ...
				  struct isp_3dnr_store *nr3_store)
{
	...
	if (!nr3_store->data_10b) {
		ISP_REG_MWR(idx, ISP_3DNR_BWD_PARA, BIT_0, 0);
		ISP_REG_MWR(idx, ISP_3DNR_BWD_PARA, BIT_4, 1 << 4);
	}
...
{
	int ret = 0;
	struct isp_dev_3dnr_info_v1 *pnr3 = NULL;
