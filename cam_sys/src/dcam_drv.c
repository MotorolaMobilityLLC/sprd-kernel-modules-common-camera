...
#undef pr_fmt
#endif
#define pr_fmt(fmt) "DCAM_DRV: %d %d %s : " fmt, current->pid, __LINE__, __func__
			...
			uint32_t *dcam_count)
{
	...
	ret = hw_info->cam_ioctl(hw_info, CAM_HW_GET_CSI_SWITCH, dn);
	if (ret)
		return ret;
	...
	pr_info("dev: %s, full name: %s, cam_ahb_gpr: %px, count: %u \n",
