...
static int sprd_flash_aw3641_open_preflash(void *drvd, uint8_t idx)
{
	...
	idx = drv_data->torch_led_index;
	ret = sprd_flash_aw3641_open_pwm(drv_data, idx, g_pre_level);
	if (ret)
	    goto exit;
...
static int sprd_flash_aw3641_cfg_value_preflash(void *drvd, uint8_t idx,
				struct sprd_flash_element *element)
{
	int ret = 0;
	g_pre_level = element->index;
	return ret;
}

static int sprd_flash_aw3641_cfg_value_highlight(void *drvd, uint8_t idx,
				struct sprd_flash_element *element)
{
	int ret = 0;
