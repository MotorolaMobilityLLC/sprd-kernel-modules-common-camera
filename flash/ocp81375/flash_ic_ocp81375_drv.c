...
#define FLASH_TEST
#define FLASH_IC_DRIVER_NAME "sprd_ocp81375"
...
static int sprd_flash_ic_init(void *drvd)
{
	...
	if (drv_data->i2c_info) {
		...
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x03, 0x9f);
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x04, 0x1e);
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x05, 0x89);
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x06, 0x0b);
	}
	return 0;
exit:
					  ...
					  struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
		flash_ic_driver_reg_write(drv_data->i2c_info, 0x05, 0x89);//78mA
		// Set Preflash Timing 0ms(No Ramp),Set highlight  Timing 400ms
		//flash_ic_driver_reg_write(drv_data->i2c_info, 0x06, 0x13);
	}
				...
				const struct i2c_device_id *id)
{
	...
	if(flash_id == 0x3a){
		pr_info("found Flashlight is flash-ocp81375.\n");
	}else{
